...
Player::~Player(){
    ...
    //Elimina el tablero
    Board = NULL;
    //Elimina los botones del borde
    boardButtons = NULL;
}

//Método que ejecuta el proceso tras la carga del núcleo y el que a su vez, genera todos los demás procesos
void Player::initTeam() {
    if (topSide) {
        //----------------------------EQUIPO DE LAS FICHAS NEGRAS----------------------------\\
        //Hace retroceder el nuevo chip creado con el constructor por defecto
...
int Player::pieceTeamIndexByXY(int x, int y) {
    // Obtiene el index de la pieza a partir de su ubicación "x-y" en el tablero
    int index=0;
